...
# `LATCHKEY_CURL` wins; `DATALIB_RUNTIME_DIR` relocates the runtime
...
runtime="${DATALIB_RUNTIME_DIR:-$here/../runtime}"
node="$runtime/node/bin/node"

if [ ! -x "$node" ]; then
    echo "latchkey: bundled node not found at $node (run datalib/tauri/stage-runtime.sh)" >&2
    exit 1
fi
...
if [ -z "$entry" ]; then
    echo "latchkey: no latchkey tree under $runtime/latchkey (run datalib/tauri/stage-runtime.sh)" >&2
    exit 1
fi
