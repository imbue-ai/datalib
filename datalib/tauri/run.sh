#!/usr/bin/env bash
# Build the Datalib desktop app and launch it — one command.
#
...
# builds `datalib-http` — UI embedded — plus the `datalib-dag`
...
# (datalib-http-<pid>.log — see `start_backend` in src/main.rs).
...
exec "$target_dir/debug/datalib-tauri" "$@"
