...
test("the grid renders both flags as check marks", async ({ page }) => {
  await page.goto("/");
  await page
  ...
  // Both columns exist and are visible by default.
  await expect(
  ...
  // The badge state arrives on a second request after the rows land,
  // so wait for the placeholder em dash to resolve rather than
  // asserting on whatever the first paint happened to show.
  ...
  // The summary line next to the row count agrees with the columns.
