...
describe("replaceToken", () => {
  it("puts the token where the key's word was, and keeps the rest", () => {
    expect(replaceToken("process:http min_level:info thread:main", "min_level", "min_level:warn")).toBe(
      "process:http min_level:warn thread:main",
    );
  });

  it("appends when the key was absent, and drops every one of it when the token is null", () => {
    expect(replaceToken("process:http", "min_level", "min_level:warn")).toBe("process:http min_level:warn");
  ...
  it("leaves a negated word alone: that is a different filter", () => {
    expect(replaceToken("-min_level:warn", "min_level", "min_level:info")).toBe("-min_level:warn min_level:info");
  });
});
