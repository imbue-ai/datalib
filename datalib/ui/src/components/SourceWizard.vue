...
///
/// Over `shownFields`, not over the whole descriptor: a dialog can only
/// require what it is asking for. The descriptor's fields span *both*
/// steps, and the two are written by two different dialogs — so
/// checking all of them made the render dialog refuse to submit until
/// a download-phase field it never showed was filled in. Signal is the
/// live case: its render step has a `period` option, so it gets a
/// second dialog, and that dialog held out for the `snapshot_dir`
/// belonging to the fetch step next door. The same gate covers the
/// `requires` case for free — a field behind a shut gate isn't shown,
/// so it can't be demanded either.
