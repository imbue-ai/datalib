...
# Prints "<total_estimated> <space-separated distinct providers>" for a
# query, retrying while the applet behind the grid is still starting.
rows() {
    local q="$1" i out
...
print(d["total_estimated"], " ".join(sorted({r["provider"] for r in d["rows"]})))
...
doc_test_block() { echo; echo "doc_test: ### $1"; }

doc_test_after() {
    case "$1" in
        demo-serve)
            wait_healthy datalib-demo
            assert_rows "" 50 claude email pdf linkedin
...
awk '
    ...
    want != "" && /^```sh/                 { block = want; want = ""; skip = (block == "setup");
                                             if (!skip) print "doc_test_block \"" block "\""; next }
    want != ""                             { want = "" }
    block != "" && /^```/                  { if (!skip) print "doc_test_after \"" block "\""; block = ""; next }
    block != "" && !skip                   { print }
' "$DOC" > "$script"
grep -q '^doc_test_block' "$script" || { echo "doc_test: no marked blocks found in $DOC" >&2; exit 1; }
