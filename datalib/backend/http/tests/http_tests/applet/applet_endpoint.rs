...
use super::{applet_command, seed_doc};
use crate::support::{get_json, state_with_applets, TEST_TOKEN};
...
async fn state_bounded(
    ...
    let applets = AppletRegistry::from_data_root_bounded(root, None, start_timeout);
    state_with_applets(root, applets).await
}
