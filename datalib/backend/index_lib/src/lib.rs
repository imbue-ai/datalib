...
//! ```
//!
//! `GridRow` and `EdgeRow` are re-exported from `datalib-schema`
//! (the autogenerated row types whose SQL DDL backs the
//! `backend_index.doltlite_db` tables). Re-exporting them here means
//! a provider's render code only imports `datalib_index_lib`:
//!
//! ```ignore
//! use datalib_index_lib::{emit_sidecar, EdgeRow, GridRow};
//! ```
...
pub use datalib_schema::edges::EdgeRow;
pub use datalib_schema::grid_rows::GridRow;
