...
/// to "what we got so far" rather than failing the download.
    ...
    match read_only_pool(path).await {
        ...
        Err(e) => {
            tracing::debug!(
                path = %path.display(),
                error = %format!("{e:#}"),
                "download_metrics: read-only snapshot open failed; bytes-only"
            );
        }
    }
...
/// The full per-source download report: API call count, the before/after
/// picture of each raw-store db file, and every WARN/ERROR the source
/// emitted during the download.
#[derive(Debug, Clone)]
pub struct DownloadReport {
    pub api_requests: u64,
    pub rows_upserted_total: u64,
    pub dbs: Vec<DbFileReport>,
    /// WARN/ERROR `tracing` events captured during this source's download
    /// (wire-level + provider-internal), in emission order. Collected by
    /// the [`datalib_obs::diagnostics`] layer; surfaced so the JSON
    /// sync summary records exactly what went wrong per source.
    pub diagnostics: Vec<datalib_obs::diagnostics::DiagnosticEntry>,
...
pub async fn assemble_report(
    ...
    diagnostics: &datalib_obs::diagnostics::Diagnostics,
) -> DownloadReport {
    let blobs_path = crate::blob_cas::cas_path_for(entity_path);
