...
//!   freshness queries). The render side sources `GridRow.when_ts`
//!   from these — typically `notes[].updated_at` for the note family,
//!   `payload.updated_at` for the MR family.
//!
//! - **String PKs, no UUIDv5 in download.** Raw-store PKs are
...
//!   `GridRow.git_sha` (render-side) and the MR's numeric `iid`
...
/// timestamp — render sources it into `GridRow.when_ts` for the
/// MR family.
///
/// Columns:
...
///   PK that identifies the project. Promoted so render / indexer
...
///   the MR merges. Preserved for render so a `merged_at`-shaped
...
/// freshness queries (and the render side's `GridRow.when_ts`
...
/// time.
///
/// Lives here next to the schema so writers (download upsert) and
/// readers (render's fixture-replay synthesizer in
/// `crate::synthesize`) agree on the exact format — same pattern as
/// signal's [`chat_item_id_recipe`].
///
/// [`chat_item_id_recipe`]: ../../../signal/src/download/schema_raw.rs
