...
use datalib_whatsapp_backup::encrypt_to_crypt15;
...
struct MediaSpec {
    mime_type: String,
    /// As msgstore spells it: relative to the backup root, so
    /// `Media/WhatsApp Images/….jpg`. Render joins this against
    /// `wa_media_files.relative_path`, which is anchored the same way.
    file_path: String,
    file_size: i64,
...
struct MediaFileSpec {
    /// Where to write the placeholder *below* `Media/` — not the
    /// `MediaSpec::file_path` form, which carries that prefix.
    relative_path: String,
    /// Placeholder file is generated by writing `size_bytes` of `'.'`
    /// — exact contents don't matter; the extractor only hashes them.
    size_bytes: usize,
}
...
fn main() -> Result<()> {
    ...
    // (see datalib/backend/clippy.toml — the rest of the code goes
