...
use datalib_etl_facebook::ingest::schema_raw::{ns_id, ALBUMS_TABLE};
...
pub fn albums_profile() -> RenderProfile {
    profile("Facebook Album", "Facebook Album Message")
}
...
fn album(row_id: &str, v: &Value, owner: &Owner) -> NormalizedChat {
    ...
    if let Some(description) = str_field(v, "description") {
        items.push(NormalizedChatItem {
            message_uuid: ns_id(&format!("msg:{id}:description")),
            author_id: "me".to_string(),
            author_display: owner.name.clone(),
            date_ms: first_photo_ms.or_else(|| ts_ms(v, "last_modified_timestamp")),
    ...
    for (i, photo) in photos.iter().enumerate() {
        ...
        items.push(NormalizedChatItem {
            message_uuid: ns_id(&format!("msg:{id}:photo:{uri}")),
            author_id: "me".to_string(),
            author_display: owner.name.clone(),
            date_ms: ts_ms(photo, "creation_timestamp"),
