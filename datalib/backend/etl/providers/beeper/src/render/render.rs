...
fn render_one(
    ...
    on_doc_complete(RenderedMarkdown {
        ...
        // doltlite download phase rewrites every doc's `events`
...
/// in the message JSON, but we don't have that handy at render
...
fn build_grid_rows(
    ...
    rows.push(
        GridRow::builder()
            ...
            .source_native_id(room.external_room_id.clone())
    ...
    for (idx, m) in doc.messages.iter().enumerate() {
        rows.push(
            GridRow::builder()
                ...
                .source_native_id(m.external_event_id.clone())
                .markdown_uuid(Some(markdown_uuid.to_string()))
                .build()?,
        );
    }
        ...
        for r in rs {
            ...
            rows.push(
                GridRow::builder()
                    ...
                    .qmd_path(qmd_path.clone())
                    .source_native_id(r.external_event_id.clone())
                    .markdown_uuid(Some(markdown_uuid.to_string()))
                    .build()?,
            );
        }
