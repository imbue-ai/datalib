...
/// `grid_rows.source_entity_kind` values — the `entity_kind` component
...
pub fn rows_for_document(meta: &DocumentMeta<'_>, pages: &[(u32, String)]) -> Vec<GridRow> {
    ...
    rows.push(GridRow {
        ...
        // under two scanned trees is deliberately one row. `source_scope`
        // stays NULL because a content hash needs no further scoping.
        source_native_id: Some(meta.blake3.to_string()),
        source_entity_kind: Some(ENTITY_KIND_DOCUMENT.to_string()),
        source_scope: None,
    ...
    for (number, text) in pages {
        ...
        rows.push(GridRow {
            ...
            source_url: source_url.clone(),
            git_sha: None,
            source_native_id: Some(format!("{}#{number}", meta.blake3)),
            source_entity_kind: Some(ENTITY_KIND_PAGE.to_string()),
            source_scope: None,
            notion_page_uuid: None,
            notion_block_uuid: None,
            markdown_uuid: Some(doc_uuid.clone()),
