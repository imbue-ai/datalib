//! `chatgpt-ingest` — incrementally mirror chatgpt.com conversations
...
async fn main() -> Result<()> {
    let args = Args::parse();
    let _guard = init_obs(&args.obs, "chatgpt-ingest")?;
    ...
    let span = info_span!("chatgpt_ingest", out = %args.out.display());
