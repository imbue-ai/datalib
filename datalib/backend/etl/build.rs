...
//! Stamps `datalib-etl`'s binaries (notably `latchkey-curl-impersonate`)
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=DATALIB_VERSION");
    println!(
        "cargo:rustc-env=DATALIB_GIT_HASH={}",
        git("rev-parse", &["HEAD"])
    );
    println!(
        "cargo:rustc-env=DATALIB_VERSION={}",
