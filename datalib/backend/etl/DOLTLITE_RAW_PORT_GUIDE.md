...
`datalib/backend/etl/src/doltlite_raw.rs` module docs):
...
Handled inside `datalib_etl::doltlite_raw::open()`:
...
`datalib_etl::doltlite_raw` owns everything provider-agnostic.
...
### Crate code (`datalib/backend/etl/providers/<name>/`)

1. **`src/extract/db.rs`** — provider-specific tables only. Shape:

   ```rust
   use datalib_etl::blob_cas::{self, BlobBundle, BlobCas, RefStub};
...
4. **`src/translate/parse.rs`** — dispatch on path:
   ...
   **The last branch returns empty, it does not `bail`.** A source with
   ...
   Build a per-bucket
   `HashMap<bucket_key, datalib_etl::blob_cas::BlobBundle>` (or
...
9. Add `"@datalib_crates//:sqlx"` to the lib's `deps`.
10. Add `"@datalib_crates//:tokio"` to the `<name>_translate` bin
    `deps` (it's now async).
...
### Sync orchestrator (`datalib/backend/sync/src/main.rs`)
...
17. **`fixture_db_snapshot__fixture_backend_index.snap`** (in
    `datalib-core`): the per-conversation/page `source_fingerprint`
...
rust_test(
    name = "foo_render",
    data = [":tng_fixture"],
    env = {"FOO_FIXTURE_DIR": "datalib/.../fixtures/foo_api"},
    ...
)
...
insta_update(
    ...
    extra_env = {"FOO_FIXTURE_DIR": "datalib/.../fixtures/foo_api"},
)
```
...
bazelisk test //datalib/backend/etl/providers/<name>/...

# 2. Round-trip via playback:
bazelisk test //datalib/backend/etl/providers/<name>:<name>_playback_roundtrip
