...
| the server, per request | nothing — [`http/src/request_log.rs`](../../datalib/backend/http/src/request_log.rs) does it | `target:http.request`: method, path, query, status, `ms`, `bytes`, and the `page` that asked; `debug` for a live refetch that succeeded (below), `info` otherwise |
