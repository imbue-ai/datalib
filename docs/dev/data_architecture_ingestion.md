...
- **`grid_index`** is provider-agnostic; it lives at [`src/grid_index.rs`](/datalib/backend/etl/src/grid_index.rs) and depends on no provider's download or render. Its input contract is the per-source render store.
...
- **`providers/<name>/src/render/schema_translate.rs`** (aspirational, landing per provider) — the normalized representation render emits: mostly serde-shaped Rust types, not SQL DDL, the in-memory POD form before it's shredded into `GridRow`s. A provider may have multiple `schema_translate_<family>.rs` files; where a shape is shared across providers (chat-human, code-review, time-series, …) the canonical type lives in a shared crate and the per-provider file re-exports.
...
- **Render-side dedup**: each document's `markdowns` row carries a `source_fingerprint`; if it already matches, the render is skipped. The grid_index step honors the same fingerprint — and reaches a document at all only when that source's `dolt_diff` surfaced it (see `source_cursors`).
...
The `source_fingerprint` compare stays — it still gates the grid_index step ("have we already loaded this document's rows?"). `source_fingerprint` is now just the stable bucket UUID. (The sidecar tree it used to live on has since been replaced by per-source doltlite tables, and render *did* get the cursor treatment described here — but the prediction that the fingerprint compare would then "go away" was wrong. Both shipped: the cursor decides which documents the index reads, the fingerprint decides whether one it read needs writing, and the cold path needs the second. See [`data_architecture_parse_and_render.md` §2](data_architecture_parse_and_render.md#where-this-is-heading-the-artifact-becomes-a-database).) This swap moves the "what's different?" decision from Rust-computed hash trees to doltlite's native diff, which it maintains anyway for `dolt diff`. The per-row `payload_blake3` columns are gone — the `WirePayloadRow` derive no longer emits them.
