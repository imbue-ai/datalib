...
| `$DATA_ROOT`                   | `/data`                    | RW                | `config.toml`, one directory per source stanza (`<name>/raw/` + `<name>/rendered_md/`), the shared indexes under `unified_index/` (`grid/db.doltlite_db`, `qmd/`), and the server's own state under `system/`. |
