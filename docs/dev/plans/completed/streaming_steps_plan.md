...
**Status: landed (September 2026), kept as the record of what was
decided.** Every render read is pinned (`etl/src/pin.rs`, checks 4–6 of
`scripts/lint_repo.py`), producers seal as they go (`Checkpointer`),
and five downloads stream to their renders — claude, chatgpt, slack,
email and garmin (`DataProcessor::streams_output`). The scheduling half
(§ The scheduler, early passes, the held-back final pass) was replaced
by the supervisor's tick ([`../supervisor.md`](../supervisor.md)); how
the loop runs a streaming producer's consumers now is in
[`datalib/backend/dag/README.md`](../../../../datalib/backend/dag/README.md).
[§ The sink contract](#the-sink-contract) is still what the code's
comments cite. Anything below that says "today" means September 2026.
The design this implemented is [`streaming_steps.md`](streaming_steps.md).
