...
the reset env vars, exits `cancelled` on SIGINT without committing, and
emits versions where it has them (the grid index claims its dolt commit hash). Use it as the
