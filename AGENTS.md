...
- [`docs/dev/plans/supervisor.md`](docs/dev/plans/supervisor.md) — chosen over the join, being built: one supervisor library reconciles the graph, open requests (what someone asked for, and so what is in scope) and facts; the batch CLI, `datalib-dag serve` and `datalib-http` all run it; sinks first-class with one writer at a time; the same verbs for a person at the screen and an agent at a shell.
