...
`AppRepo` trait in `repo.rs`. The grid-side `DoltRepo` is a different
type in a different crate
(`datalib/backend/unified_index/src/dolt_repo.rs`) and has nothing to
do with feedback. doltlite is statically linked into every Rust binary
by `//third-party/doltlite:sqlite3` (see `MODULE.bazel`); no host
`dolt` install, no subprocess, no MySQL client. The same pool serves
reads and writes.
