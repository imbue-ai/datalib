"""Tests against the cached output of `//claude-mirror/tests/fixtures:ingested_tng`.
...
def test_dump_sql_has_expected_tables_and_rows() -> None:
    dump = _runfile("claude-mirror/tests/fixtures/ingested/dump.sql").read_text()
...
def test_qmd_tar_contains_expected_files() -> None:
    tar_path = _runfile("claude-mirror/tests/fixtures/ingested/qmd.tar")
...
def test_dump_sql_loads_into_in_memory_sqlite() -> None:
    ...
    conn = load_dump_into_memory(_runfile("claude-mirror/tests/fixtures/ingested/dump.sql"))
