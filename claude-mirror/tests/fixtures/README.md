...
```
bazelisk build //claude-mirror/tests/fixtures:ingested_tng
# bazel-bin/claude-mirror/tests/fixtures/ingested/dump.sql
# bazel-bin/claude-mirror/tests/fixtures/ingested/qmd.tar
```
...
```python
from claude_mirror.sqlite_load import load_dump_into_memory
...
1. Run `cd claude-mirror && uv run pytest tests/test_fixtures.py` —
...
**Golden snapshots.** `claude-mirror/tests/test_snapshots.py` writes
...
```
bazelisk build //claude-mirror/tests/fixtures:ingested_tng
cd claude-mirror && uv run pytest tests/test_snapshots.py --snapshot-update
```
