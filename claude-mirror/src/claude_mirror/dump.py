...
fidelity. See `claude_mirror.sqlite_load` for the SQLite loader.
...
from claude_mirror.providers.anthropic.schema import DDL as _ANTHROPIC_DDL
from claude_mirror.providers.openai.schema import DDL as _OPENAI_DDL
...
def _portable_ddl_for(table: str) -> str:
    ...
    for stmt in (*_ANTHROPIC_DDL, *_OPENAI_DDL):
        ...
        if re.search(rf"\bCREATE TABLE\b\s+(IF\s+NOT\s+EXISTS\s+)?{re.escape(table)}\b", normalized):
            return re.sub(
...
def _quote_value(v: Any) -> str:
    ...
    if isinstance(v, str):
        ...
        if stripped[:1] in ("{", "["):
            try:
                parsed = json.loads(v)
                s = json.dumps(parsed, sort_keys=True, ensure_ascii=False, separators=(",", ":"))
...
def dump_sql(conn: Connection, out_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("-- Generated by claude_mirror.dump.dump_sql")
    lines.append("-- This file is byte-stable for a given input + schema; do not hand-edit.")
    lines.append("")
