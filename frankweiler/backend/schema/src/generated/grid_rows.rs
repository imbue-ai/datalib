...
// Provider-agnostic union table that backs the AG Grid in frankweiler. Every searchable entity in the system (chat conversations, individual messages, content blocks, slack messages, ...) emits one row here at ingest time, keyed by a provider-namespaced UUID. The Rust grid backend reads this table from <root>/backend_index.doltlite_db with a single query and renders rows directly. Per-provider tables (anthropic_*, openai_*, slack_*) remain the authoritative store for raw payloads + render input; grid_rows is the denormalized projection. Single source of truth for column names, types, and per-provider mappings — codegen produces matching Python dataclass + DDL, Rust struct, and TypeScript interface.
...
pub struct GridRow {
    ...
    /// FK into the `markdowns` table — every rendered `.md` file gets one row in `markdowns`, and every grid_row that lives inside that file points at it. Many-to-1. A single upstream 'conversation' can shard across multiple markdowns when a provider renders one file per period (beeper); `markdown_uuid` is the addressing primitive the rest of the system (notably `/api/chat/{markdown_uuid}`) uses. Nullable today because the column is being introduced ahead of the producer-side population pass (see plan-ui-driven-sync.md, Phase C); will become NOT NULL once every renderer emits it. Drives incremental re-render (skip if `markdowns.row_set_hash` unchanged) and per-markdown re-ingest.
