...
//!   See `docs/dev/data_architecture_ingestion.md` §"Retry and fetch durability":
...
//!   source for `GridRow.when_ts` on the render side. Sub-items
...
//!   is the 0-based index from the BFS walk in `download::mod::walk_page_blocks`;
...
///   `GridRow.when_ts` on the render side.
...
///   per-page child join (render / render) avoids cracking the
///   payload.
/// - `page_order` — 0-based index of this block within the owning
///   page's BFS walk (see `download::mod::walk_page_blocks`). Render
///   uses it to lay out sections / toggles deterministically,
///   independent of UUID ordering. Not part of identity.
/// - `last_edited_time` — upstream `payload.last_edited_time` ISO-8601
///   stamp. Source for `GridRow.when_ts` on the render side.
...
///   omits a parent (see `download::mod::mirror_page`).
/// - `page_id` — the owning page's UUID, propagated by the writer so
///   the per-page child join (render / render) avoids cracking the
///   payload.
...
/// render side per `docs/dev/data_architecture_ingestion.md` §"Object identity".
