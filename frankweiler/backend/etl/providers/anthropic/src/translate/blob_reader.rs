//! Anthropic-specific [`BlobReader`].
//!
//! `read_by_ref_id(file_uuid)`:
//! 1. `SELECT blake3 FROM anthropic_attachments WHERE file_uuid = ?
//!    AND blake3 IS NOT NULL LIMIT 1`
...
pub struct AnthropicBlobReader {
    refs_pool: SqlitePool,
    cas_pool: SqlitePool,
}

impl AnthropicBlobReader {
    ...
    async fn read_by_ref_id_async(&self, ref_id: &str) -> Result<Option<BlobView>> {
        let blake3: Option<String> = sqlx::query_scalar(
            "SELECT blake3 FROM anthropic_attachments \
             WHERE file_uuid = ? AND blake3 IS NOT NULL LIMIT 1",
        )
        .bind(ref_id)
        .fetch_optional(&self.refs_pool)
        .await
        .with_context(|| format!("lookup anthropic_attachments by file_uuid {ref_id}"))?;
...
impl BlobReader for AnthropicBlobReader {
