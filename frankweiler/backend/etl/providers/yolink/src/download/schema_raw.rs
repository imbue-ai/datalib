...
//!   The signing happens in [`crate::download`] (it touches secrets);
...
///
/// Used by `download::RawDb::reset` to wipe per-row state without
...
///   the per-run resume cursor (see [`crate::download::fetch_device`]).
...
///   render / downstream side uses as `GridRow.when_ts`.
