...
pub fn parse(body: &str, kind: &str) -> Result<Vec<Reading>> {
    ...
    let val_idxs: Vec<usize> = cols.iter().map(|(h, _, _)| find(h)).collect::<Result<_>>()?;

    let mut out = Vec::new();
    for (i, rec) in rdr.records().enumerate() {
        let rec = rec.with_context(|| format!("row {}", i + 2))?;
        let Some(ts) = rec.get(time_idx) else { continue };
        ...
        for ((_, metric, suffix), &idx) in cols.iter().zip(&val_idxs) {
            let Some(raw) = rec.get(idx).filter(|s| !s.is_empty()) else { continue };
            ...
            let numeric = raw.strip_suffix(suffix).ok_or_else(|| {
                anyhow!("row {} {metric}: value {raw:?} missing suffix {suffix:?}", i + 2)
            })?;
            let value = numeric
                .parse::<f64>()
                .with_context(|| format!("row {} {metric}: parse {numeric:?}", i + 2))?;
            out.push(Reading { ts_ms, metric, value });
        }
    }
    Ok(out)
}
...
pub async fn fetch(opts: FetchOptions) -> Result<FetchSummary> {
    ...
    opts.progress.set_length(Some(opts.sync.devices.len() as u64));
...
async fn fetch_device(
    ...
    while cursor < now_ms {
        ...
        let body = curl(&url).await.with_context(|| format!("{} {cursor}..{end}", dev.name))?;
        s.requests += 1;
        s.windows += 1;
        let rows = parse(&body, &dev.kind)
        ...
        let msg = format!("yolink {} [{cursor}..{end}]: +{touched} ={unchanged}", dev.name);
...
async fn curl(url: &str) -> Result<String> {
    ...
    out.status.success().then_some(()).ok_or_else(|| {
        anyhow!("curl exit {}: {}", out.status, String::from_utf8_lossy(&out.stderr).trim())
    })?;
...
mod tests {
    ...
    async fn upsert_distinguishes_new_changed_unchanged() {
        ...
        let db = RawDb::open(&dir.path().join("yl.doltlite_db")).await.unwrap();
        let pool = db.pool();
        let r = |ts, v| Reading { ts_ms: ts, metric: "water_meter_gal", value: v };
        assert_eq!(upsert_readings(pool, "v", &[r(100, 1.0), r(200, 2.0)]).await.unwrap(), (2, 0));
        assert_eq!(upsert_readings(pool, "v", &[r(100, 1.0), r(200, 2.0)]).await.unwrap(), (0, 2));
        assert_eq!(upsert_readings(pool, "v", &[r(100, 1.5), r(200, 2.0)]).await.unwrap(), (1, 1));
    }
}
