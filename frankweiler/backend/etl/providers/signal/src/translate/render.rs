...
fn render_markdown(
    ...
    for (idx, item) in doc.items.iter().enumerate() {
        ...
        // `id` + `data-section-uuid` give the frontend an anchor for
        // scroll-into-view / .selected when a grid row whose
        // message_grid_row.uuid is `msg_uuid` is clicked. Every other
        // provider does the same thing on its message-level wrapper
        // div; signal uses an inline span on the bullet so the
        // markdown shape (bulleted list of timestamps) stays intact.
        // The span wraps visible content (timestamp + author) so the
        // browser gives it non-zero size — an empty anchor span would
        // be `hidden` to Playwright's `toBeVisible` check and the
        // selection highlight would have no surface to land on.
        s.push_str(&format!(
            "- <span id=\"m-{msg_uuid}\" data-section-uuid=\"{msg_uuid}\" \
             data-msg-index=\"{idx}\">**{ts}** _{author}_:</span> {body}\n"
        ));
