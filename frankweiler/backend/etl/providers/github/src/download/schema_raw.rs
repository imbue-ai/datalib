...
//! - **Render-side UUIDs are a separate recipe.** Cross-provider grid
//!   UUIDs (`github:{repo}:pr:{number}`, `github:{repo}:issue_comment:{id}`,
//!   …) are UUIDv5-derived in `crate::render::parse`
...
//!   render concerns and don't appear in the raw store; the raw PKs
...
//!   children carry `created_at` / `updated_at`. Render sources
//!   `GridRow.when_ts` from `updated_at` on `pull_requests` and from
//!   `created_at` on the comment / review children.
//! - **Refresh-window cursor strategy.** Download does not stream every
...
//!   `download::since_for_scope`. The cursor itself lives in
...
/// capture; the render side reads it to label the `_source` block on
/// every grid row.
///
...
///   Sourced into `GridRow.when_ts` by render.
...
/// render-side cross-provider grid UUIDs
/// (`github_pr_uuid`, …) live in `crate::render::parse` and key off
