...
impl GridIndex {
    ...
    /// Build from `<root>/mirror.sqlite`'s `grid_rows` table.
    pub fn from_sqlite(conn: &Connection) -> Self {
        let mut rows: Vec<GridRowRef> = Vec::new();
        if let Ok(mut stmt) =
            conn.prepare("SELECT uuid, kind, COALESCE(qmd_path, ''), provider FROM grid_rows")
        {
