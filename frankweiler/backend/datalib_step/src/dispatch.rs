...
//!
//! With per-provider step types (`slack_api.download`,
//! `slack_api.render`, …) the params no longer carry a `type:` tag —
//! the step type names the provider, and the `source:` subtree
//! deserializes straight into that provider's own config struct. Each
//! arm normalizes the config exactly like the old `Config::normalize`
//! (fold built-in defaults, resolve paths against the data root),
//! validates it, and calls the provider's per-wave entry point
...
pub fn plan(
    ...
    macro_rules! arm {
        ($cfgty:ty, $provider:ident, $tstr:expr) => {{
            let mut cfg: $cfgty = serde_json::from_value(source)
                .with_context(|| format!("parse params.source as a {} config", $tstr))?;
            // No global `defaults:` stanza in DAG mode (each step is
            // self-contained): fold the built-in defaults only.
            ...
            let processors = match phase {
                Phase::Download => $provider::processor::plan_download(ctx, cfg)?,
                Phase::Render => $provider::processor::plan_render(ctx, cfg)?,
            };
            PlannedSource {
                name: name.to_string(),
                type_str: $tstr,
...
mod tests {
    ...
    #[test]
    fn plans_slack_download_and_render_from_untagged_source() {
        ...
        let rn = plan("slack_api", Phase::Render, "slack", source, td.path()).unwrap();
