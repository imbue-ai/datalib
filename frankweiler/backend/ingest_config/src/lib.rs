...
impl SourceConfig {
    ...
    pub fn is_managed(&self) -> bool {
        match self {
            ...
            // Managed via `sync:` (local discovery/import, primary UX) or an
            // explicit `input_path:` export directory (advanced fallback).
            SourceConfig::Hermes(c) => c.sync.is_some() || c.common.input_path.is_some(),
