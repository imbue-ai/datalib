...
from snapshot_extensions import MarkdownSnapshotExtension, SqlSnapshotExtension
...
def _locate_artifact() -> Path | None:
    ...
    if runfiles:
        cand = Path(runfiles) / "_main" / "tests" / "fixtures" / "ingested"
        if (cand / "dump.sql").exists():
            return cand
    # Fallback for `uv run pytest`: walk up to a `bazel-bin/` sibling.
    here = Path(__file__).resolve()
    for parent in here.parents:
        cand = parent / "bazel-bin" / "tests" / "fixtures" / "ingested"
        if (cand / "dump.sql").exists():
            return cand
    return None
...
pytestmark = pytest.mark.skipif(
    ...
    reason="ingested_tng artifact not built; run `bazelisk build //tests/fixtures:ingested_tng`",
)
